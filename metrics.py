...
def get_average_number_of_key_elements_by_class(labels, classes):
    ...
    return np.average(num_key_elements_correct) if num_key_elements_correct else 0.0, np.average(num_key_elements_partial) if num_key_elements_partial else 0.0, np.average(num_key_elements_incorrect) if num_key_elements_incorrect else 0.0
...
def get_average_realtion_by_class(labels, classes):
    ...
    return np.average(relations_correct) if relations_correct else 0.0, np.average(relations_partial) if relations_partial else 0.0, np.average(relations_incorrect) if relations_incorrect else 0.0
...
def get_average_number_of_tokens_per_key_element_by_class(labels, classes):
    ...
    return np.average(num_tokens_correct) if num_tokens_correct else 0.0, np.average(num_tokens_partial) if num_tokens_partial else 0.0, np.average(num_tokens_incorrect) if num_tokens_incorrect else 0.0
...
def compute_metrics(outputs):
    ...
    tn_correct, tn_partial, tn_incorrect = get_average_number_of_tokens_per_key_element_by_class(true_predictions, classes)
    metrics = {
